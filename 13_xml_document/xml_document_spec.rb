...
describe XmlDocument do
  ...
  it "renders an empty tag" do
    expect(@xml.hello).to eql("<hello/>")
  end

  it "renders a tag with attributes" do
    expect(@xml.hello(:name => 'dolly')).to eql("<hello name='dolly'/>")
  end

  it "renders a randomly named tag" do
    tag_name = (1..8).map{|i| ('a'..'z').to_a[rand(26)]}.join
    expect( @xml.send(tag_name)).to eql("<#{tag_name}/>")
  end

  it "renders block with text inside" do
    expect(@xml.hello do
      "dolly"
    end).to eql( "<hello>dolly</hello>" )
  end

  it "nests one level" do
    expect( @xml.hello do
      @xml.goodbye
    end ).to eql("<hello><goodbye/></hello>")
  end
  ...
  it "nests several levels" do
    ...
    end).to eql( "<hello><goodbye><come_back><ok_fine be='that_way'/></come_back></goodbye></hello>")
  end
